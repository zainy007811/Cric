...
const ProfessionalHlsPlayer: React.FC<ProfessionalHlsPlayerProps> = ({ src, title, startUnmuted = false }) => {
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on('hlsMediaAttached', () => {
        ...
        // Autoplay might be blocked if not muted, so we try and handle the error.
  ...
  return (
    <div ref={playerContainerRef} className="relative w-full bg-black group select-none" style={{ aspectRatio }} onMouseMove={showControls} onClick={() => isLocked ? setIsLocked(false) : showControls()}>
      <video ref={videoRef} className={`w-full h-full ${isCasting ? 'opacity-0' : ''}`} style={{ filter: `brightness(${brightness})` }} onClick={(e) => { e.stopPropagation(); if (!isLocked) togglePlayPause(); }} playsInline />
        ...
        {!isLocked && (
          <>
            ...
            <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-10">
              ...
              {isCastAvailable && <google-cast-launcher style={{ width: '24px', height: '24px', '--cast-button-color': 'white' }}></google-cast-launcher>}
            </div>
